...
│                                                                          │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────────┐  │
│  │  specs/  │ │  infra/  │ │ scripts/ │ │  docs/   │ │ .github/     │  │
│  │ API契约  │ │ Docker   │ │ 部署/测试 │ │ 设计文档  │ │ CI/CD 流水线 │  │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────────┘  │
└──────────────────────────────────────────────────────────────────────────┘
```
...
| `scripts/run-tests.ps1` | RAG 回归测试（55 用例） | __________ |
| `scripts/quality.ps1` | 一键代码质量检查（lint + test 三服务） | __________ |
| `.github/workflows/ci.yml` | GitHub Actions CI（前端/后端/Python 自动质量检查） | __________ |
...
| 工具 | 覆盖 | 说明 |
|------|------|------|
| Ruff | Python | lint + format，128 处自动修复 |
| ESLint | TypeScript | 前端代码规范，0 错误 |
| Vitest | TypeScript | 前端单元测试，3 用例 |
| Checkstyle | Java | 代码风格检查 |
| JaCoCo | Java | 测试覆盖率报告 |
| pytest | Python | 55 单元 + 回归 + 33 集成测试 |
