...
class RegressionTests(unittest.TestCase):
    ...
    def _run_one(self, idx: int, question: str, session_id: str = None):
        """执行单条问题并记录结果"""
        try:
            r = self.pipeline.handle_question(
                QAAskRequest(question=question, session_id=session_id, mode="rule")
            )
            return r
        except Exception as e:
            return {"status": "error", "intent": None, "answer": "", "code": str(e), "no_data": False,
                    "error": str(e)}
    ...
    def _test_single(self, idx: int):
        ...
        self.assertIn(r.status, ("ok", "no_data", "clarify"),
                      f"#{idx+1} '{q}' → unexpected status={r.status}")

        # 如果有预期意图，校验意图匹配
        expected = EXPECTED_INTENT.get(idx)
        if expected is not None and r.status != "clarify":
            self.assertEqual(r.intent, expected,
                             f"#{idx+1} '{q}' → intent={r.intent}, expected={expected}")

    def test_01_artifact_museum(self):        self._test_single(0)
    def test_02_artifact_period(self):        self._test_single(1)
    def test_03_painting_author(self):        self._test_single(2)
    def test_04_artist_biography(self):       self._test_single(3)
    def test_05_museum_count(self):           self._test_single(4)
    def test_06_artifact_material(self):      self._test_single(5)
    def test_07_artifact_type(self):          self._test_single(6)
    def test_08_artifact_description(self):   self._test_single(7)
    def test_09_artifact_dimensions(self):    self._test_single(8)
    def test_10_same_artist_works(self):      self._test_single(9)
    def test_11_recommended_artifacts(self):  self._test_single(10)
    def test_12_dynasty_representative(self): self._test_single(11)
    def test_13_artifact_museum_en(self):     self._test_single(12)
    def test_14_artifact_period2(self):       self._test_single(13)
    def test_15_artifact_material2(self):     self._test_single(14)
    def test_16_artifact_dimensions2(self):   self._test_single(15)
    def test_17_painting_author2(self):       self._test_single(16)
    def test_18_dynasty_representative2(self): self._test_single(17)
    def test_19_museum_count2(self):          self._test_single(18)
    def test_20_same_artist_works2(self):     self._test_single(19)
    def test_21_artifact_type2(self):         self._test_single(20)
    def test_22_artifact_description2(self):  self._test_single(21)
    def test_23_artist_biography2(self):      self._test_single(22)
    def test_24_museum_count3(self):          self._test_single(23)
    def test_25_artifact_museum_qmsht(self):  self._test_single(24)
    def test_26_artifact_period_nszht(self):  self._test_single(25)
    def test_27_dynasty_tang(self):           self._test_single(26)
    def test_28_compare_artifacts(self):      self._test_single(27)
    def test_29_artifact_statistics(self):    self._test_single(28)
    def test_30_path_query(self):             self._test_single(29)
    def test_31_multi_hop(self):              self._test_single(30)
    ...
    def test_32_pronoun_resolution(self):
        ...
        self.assertIn(r2.status, ("ok", "no_data", "clarify"),
                      f"#32 pronoun '{q}' → status={r2.status}")
    ...
    def test_33_topic_switch(self):
        ...
        self.assertIn(r2.status, ("ok", "no_data", "clarify"),
                      f"#33 topic switch '{q}' → status={r2.status}")
        self.assertEqual(r2.intent, "painting_author",
                         f"#33 expected painting_author, got {r2.intent}")
    ...
    def test_no_data_unknown_entity(self):
        ...
        for q in ["一块不知名石头的材质是什么？", "abcdefg在哪个博物馆？", "不存在的文物123的介绍"]:
            ...
            self.assertNotEqual(r.status, "error",
                              f"'{q}' should not crash, got status={r.status}")
